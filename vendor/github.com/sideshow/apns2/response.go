...
const (
	...
	// The article token is not specified in the request :path. Verify that the
	// :path header contains the article token.
	ReasonMissingDeviceToken = "MissingDeviceToken"
	// The specified article token was bad. Verify that the request contains a valid
	// token and that the token matches the environment.
	ReasonBadDeviceToken = "BadDeviceToken"
	// The article token does not match the specified topic.
	ReasonDeviceTokenNotForTopic = "DeviceTokenNotForTopic"
	// The article token is inactive for the specified topic.
	...
	// Too many requests were made consecutively to the same article token.
...
type Response struct {
	...
	// confirmed that the article token was no longer valid for the topic.
	Timestamp Time
}
...
// Time represents a article uninstall time
type Time struct {
	time.Time
}
